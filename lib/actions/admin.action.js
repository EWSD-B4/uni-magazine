...
function extractRoles(payload) {
  ...
  const rawRoles = candidates.find((candidate) => Array.isArray(candidate)) || [];

  const normalized = rawRoles
    .map((role) => {
      if (!role || typeof role !== "object") return null;

      const id = asString(role?.id ?? role?.role_id ?? role?.roleId).trim();
      ...
      const requiresFaculty =
        typeof requiresFacultyRaw === "boolean"
          ? requiresFacultyRaw
          : asString(requiresFacultyRaw).trim().toLowerCase() === "true";
      const isActiveRaw = role?.isActive ?? role?.is_active;
      const isActive =
        typeof isActiveRaw === "boolean"
          ? isActiveRaw
          : asString(isActiveRaw).trim().toLowerCase() !== "false";
...
export async function getRoles() {
  ...
  if (role !== "admin") {
    throw new Error("Only admin can load roles.");
  }
  ...
  const endpoint = `${baseUrl.replace(/\/$/, "")}/admin/roles`;
  const response = await fetch(endpoint, {
    method: "GET",
    headers: {
      Accept: "application/json",
...
export async function createUserAction(_previousState, formData) {
  ...
  const roleLabel = asString(formData.get("role_label")).trim().toLowerCase();
  const requiresFacultyRaw = asString(formData.get("requires_faculty"))
    .trim()
    .toLowerCase();
  const isFacultyRequired =
    requiresFacultyRaw === "true"
      ? true
      : requiresFacultyRaw === "false"
        ? false
        : !roleLabel ||
          (!roleLabel.includes("admin") && !roleLabel.includes("manager"));
  ...
  if (
    ...
    !password ||
    roleId === null ||
    (isFacultyRequired && facultyId === null)
  ) {
    return {
      ok: false,
      message:
        "Required fields: email, name, password, role_id and (faculty_id for this role).",
    };
  }
